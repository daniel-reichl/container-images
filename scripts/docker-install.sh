...
usage() {
  ...
  echo ""
  echo "Install Docker Engine and/or configure pass-based credential storage."
  echo ""
  echo "Options:"
  echo "  --help        Show this help message and exit"
  echo "  --pass-only   Only set up Docker credential helper (pass), skip Docker install"
  echo "  --skip-pass   Install Docker only, skip credential helper (pass) setup"
  echo ""
  echo "With no options, performs a full install (Docker + pass)."
}
...
if [[ "$INSTALL_DOCKER" == false && "$INSTALL_PASS" == false ]]; then
  echo "Error: --pass-only and --skip-pass cannot be used together."
  exit 1
fi
...
fi # end INSTALL_DOCKER

# ============================================================
#  DOCKER CREDENTIAL HELPER (PASS) SETUP
# ============================================================
if [[ "$INSTALL_PASS" == true ]]; then

# ------------------------------------------------------------
#  Install Dependencies (pass-only mode)
# ------------------------------------------------------------
if [[ "$INSTALL_DOCKER" == false ]]; then
  ...
  sudo apt-get install -y gpg pass curl
fi
...
# ------------------------------------------------------------
echo ""
echo ">>> Installing Docker credential helper (docker-credential-pass)..."
CRED_HELPER_VERSION=$(curl -fsSL https://api.github.com/repos/docker/docker-credential-helpers/releases/latest | grep '"tag_name"' | sed 's/.*"v\([^"]*\)".*/\1/')
if [ -z "$CRED_HELPER_VERSION" ]; then
  echo "WARNING: Could not fetch latest version from GitHub. Falling back to 0.8.2."
  CRED_HELPER_VERSION="0.8.2"
fi
echo "Using docker-credential-pass version: ${CRED_HELPER_VERSION}"
...
# ------------------------------------------------------------
#  Set Up GPG Key and Pass for Docker Credential Storage
# ------------------------------------------------------------
echo ""
echo ">>> Configuring GPG and pass for Docker credential storage..."
...
echo ""
echo "------------------------------------------------------------"
if [[ "$INSTALL_DOCKER" == true && "$INSTALL_PASS" == true ]]; then
  echo "Docker installation script finished successfully!"
elif [[ "$INSTALL_DOCKER" == true ]]; then
  echo "Docker installation finished successfully! (pass setup skipped)"
else
  echo "Docker credential helper (pass) setup finished successfully!"
fi
echo "------------------------------------------------------------"
echo ""
